...
def create_virt_net(nodes_count, port_move=0):
    ...
    for unuse in range(nodes_count):
        ...
        args = ['/usr/bin/python', FABNET_NODE_BIN, address, n_node, '%.02i'%i, home, 'DHT', '--nodaemon']
        #if DEBUG:
        #args.append('--debug')
...
def collect_topology_from_nodes(addresses):
    for address in addresses:
        p = subprocess.Popen(['/usr/bin/python', FRI_CALLER_BIN, 'TopologyCognition', address, '{"need_rebalance": 1}', 'async'])
